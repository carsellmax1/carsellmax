...
export async function POST(request: NextRequest) {
  try {
    ...
    const initialValuation = {
      ...
      version: 1,
      status: 'draft',
      vehicle_year: submission.vehicles?.year || 0,
      vehicle_make: submission.vehicles?.make || '',
      vehicle_model: submission.vehicles?.model || '',
      vehicle_vin: submission.vehicles?.vin || null,
      vehicle_mileage: submission.vehicles?.mileage || 0,
      vehicle_color: submission.vehicles?.color || null,
      vehicle_condition: submission.vehicles?.condition || 'good',
      base_value: 0,
